...
#########################################################################
if [[ $TAG_NAME_VALUE =~ doppio-data[0][1-4]-2b ]]; then
  sudo puppet apply --certname=doppio-b-data --modulepath=/goshposh/sysops/aws/puppet/modules /goshposh/sysops/aws/puppet/manifests/nodes.pp
fi
#########################################################################
if [[ $TAG_NAME_VALUE =~ doppio-data[0][1-4]-2c ]]; then
  sudo puppet apply --certname=doppio-c-data --modulepath=/goshposh/sysops/aws/puppet/modules /goshposh/sysops/aws/puppet/manifests/nodes.pp
fi
