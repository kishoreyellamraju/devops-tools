...
#########################################################################
if [[ $TAG_NAME_VALUE =~ espresso-data[0][1-2]-2b ]]; then
  sudo puppet apply --certname=espresso-b-data --modulepath=/goshposh/sysops/aws/puppet/modules /goshposh/sysops/aws/puppet/manifests/nodes.pp
fi
#########################################################################
if [[ $TAG_NAME_VALUE =~ espresso-data[0][1-2]-2c ]]; then
  sudo puppet apply --certname=espresso-c-data --modulepath=/goshposh/sysops/aws/puppet/modules /goshposh/sysops/aws/puppet/manifests/nodes.pp
fi
