...
#######################################################################
if [[ $TAG_NAME_VALUE =~ doppio-master[0][1-2]-2[b-c]$ ]]; then
   sudo puppet apply --certname=doppio-b-master --modulepath=/goshposh/sysops/aws/puppet/modules /goshposh/sysops/aws/puppet/manifests/nodes.pp
fi
#########################################################################
if [[ $TAG_NAME_VALUE =~ doppio-master[0][3]-2[b-c]$ ]]; then
  sudo puppet apply --certname=doppio-c-master --modulepath=/goshposh/sysops/aws/puppet/modules /goshposh/sysops/aws/puppet/manifests/nodes.pp
fi
