...
#######################################################################
if [[ $TAG_NAME_VALUE =~ breve-master[0][1-2]-2[b-c]$ ]]; then
   sudo puppet apply --certname=breve-b-master --modulepath=/goshposh/sysops/aws/puppet/modules /goshposh/sysops/aws/puppet/manifests/nodes.pp
fi
#########################################################################
if [[ $TAG_NAME_VALUE =~ breve-master[0][3]-2[b-c]$ ]]; then
  sudo puppet apply --certname=breve-c-master --modulepath=/goshposh/sysops/aws/puppet/modules /goshposh/sysops/aws/puppet/manifests/nodes.pp
fi
