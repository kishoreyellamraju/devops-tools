...
#########################################################################
if [[ $TAG_NAME_VALUE =~ romano-client[0][1-2]-2b ]]; then
  sudo puppet apply --certname=romano-b-client --modulepath=/goshposh/sysops/aws/puppet/modules /goshposh/sysops/aws/puppet/manifests/nodes.pp
fi
#########################################################################
if [[ $TAG_NAME_VALUE =~ romano-client[0][1-2]-2c ]]; then
  sudo puppet apply --certname=romano-c-client --modulepath=/goshposh/sysops/aws/puppet/modules /goshposh/sysops/aws/puppet/manifests/nodes.pp
fi
