...
#########################################################################
if [[ $TAG_NAME_VALUE =~ doppio-client[0][1-2]-2b ]]; then
  sudo puppet apply --certname=doppio-b-client --modulepath=/goshposh/sysops/aws/puppet/modules /goshposh/sysops/aws/puppet/manifests/nodes.pp
fi
#########################################################################
if [[ $TAG_NAME_VALUE =~ doppio-client[0][1-2]-2c ]]; then
  sudo puppet apply --certname=doppio-c-client --modulepath=/goshposh/sysops/aws/puppet/modules /goshposh/sysops/aws/puppet/manifests/nodes.pp
fi
